...
export default function JoinBetaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Try post-login consent first
      const res = await fetch('/api/user/consent', { method: 'POST' });
      
      if (res.ok) {
        // Successfully granted consent post-login
        router.push('/dashboard');
        return;
      }
      
      if (res.status === 401) {
        // Not logged in -> Pre-login consent flow
        document.cookie = "pre_consent=true; path=/; max-age=3600";
        sessionStorage.setItem('kaizen_consent', JSON.stringify({
          data: consentData,
        ...
        console.error("Failed to grant consent:", await res.text());
      }
    } catch (err) {
